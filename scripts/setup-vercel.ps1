...
# value). Nothing is printed to the console.
#
...
# files using the system ANSI codepage, so a UTF-8 character such as an em dash
# is mis-decoded and produces a parser error.
...
foreach ($name in ($required + $optional)) {
  if (-not $vars.ContainsKey($name)) {
    Write-Host ("  skip     {0} (not set)" -f $name)
    continue
  }
  ...
  foreach ($target in @("production", "preview")) {
    ...
    if ($LASTEXITCODE -eq 0) {
      ...
      Write-Host ("  FAILED   {0} [{1}]" -f $name, $target) -ForegroundColor Red
    }
  }
}
